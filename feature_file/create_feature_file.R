...
### get inverse predicate properties
data_inv <- read.csv('pred_property_p_50/dbp_map_inv.csv')
data_inv <- rbind(data_inv, read.csv('pred_property_p_50/dbp_raw_inv.csv'))
data_inv <- rbind(data_inv, read.csv('pred_property_p_50/wd_inv.csv'))
data_inv <- rbind(data_inv, read.csv('pred_property_p_50/fb_inv.csv'))
data_inv$pred_inv <- as.character(data_inv$pred_inv)
...
data_inv <- inner_join(data_inv, est_matches, by=c('pred_inv' = 'predicate'))
...
data_inv <- inner_join(data_inv, sub_obj_types, by=c('pred_inv' = 'predicate'))
colnames(data_inv)[13:14] <- c('obj_type', 'sub_type')
data_inv$sub_type[which(data_inv$sub_type == 'literal' | data_inv$sub_type == 'bnode')] <- 'Thing'
...
write.csv(data_inv, 'feature_file/inv_predicates_p_50.csv', row.names = F)
