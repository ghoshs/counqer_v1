...
class PostgresDB:
	...
	def __init__(self, createtb, tablename, params):
	...
	def create_pred_property_Table(self, tablename):
	...
	def create_invpred_property_Table(self, tablename):
		cur = self.conn.cursor()
...
def get_type_count(pred, db):
	cur = db.conn.cursor()
	types={}
	cur.execute("""SELECT obj_type, count(*) FROM spot_triples where pred=(%s) group by obj_type""", [pred])
	row = cur.fetchone()
...
def get_persub_val(pred, db):
	...
	types = {}
	count_list = []
	cur.execute("""select count(*) from spot_triples 
		where pred=(%s) and obj_type='int' group by sub""", [pred])
	for row in cur:
...
def get_type_numeric(pred, db):
	...
	cur.execute("""select obj from spot_triples where pred=(%s) and obj_type='int' """, [pred])
	for row in cur:
		val = 0
		try: 
			val = int(row[0])
...
def get_type_value(pred, db):
	...
	cur.execute("""select count(*) from spot_triples 
		where pred=(%s) and obj_type='named_entity' group by sub""", [pred])
	for row in cur:
		count_list.append(row[0])
	cur.close()
	types['max'] = max(count_list)
...
def get_type_comma_sep(pred, db):
	...
	freq = 0
	cur.execute("""select count(*) from spot_triples 
		where pred=(%s) and obj_type='unknown'
		and obj similar to '(\s*,\s*[^\s]+)+'""", [pred]) 
...
def update_prop_table(db, pred, freq, percent_dist_dict, numeric_dist_dict, value_dist_dict, dist_comma_sep, persub_val_int):
	cur = db.conn.cursor()
	cur.execute("""INSERT INTO pred_property
				(Predicate, Frequency, 
...
def generate_details(predfreq_filename, db, cutoff):
	...
	for i, item in tqdm(enumerate(predfreq)):
		...
		update_prop_table(db, pred, freq, percent_dist_dict, numeric_dist_dict, value_dist_dict, dist_comma_sep, persub_val_int)
		# if (i%100 == 0):
...
def get_inv_type_value(pred, db):
	...
	types = {}
	count_list = []
	cur.execute("""select count(*) from spo_inverse 
		where pred_inv=(%s) group by sub""", [pred])
	for row in cur:
		count_list.append(row[0])
	cur.close()
	if len(count_list) == 0:
...
def update_invprop_table(db, pred, value_dist_dict):
	cur = db.conn.cursor()
	cur.execute("""INSERT INTO inv_pred_property
				(pred_inv, 
				PerSub_max_NE, 
				PerSub_min_NE, 
				PerSub_avg_NE,
				...
				(pred, 
					value_dist_dict['max'], value_dist_dict['min'], value_dist_dict['avg'],
					value_dist_dict['p10'], value_dist_dict['p90'], value_dist_dict['freq']
				))
	db.conn.commit()
	cur.close()
...
def generate_inv_pred_details(predfreq_filename, db, cutoff):
	...
	for i, item in tqdm(enumerate(predfreq)):
		...
		if value_dist_dict is None:
			# value_dist_dict = {'max': None, 'min': None, 'avg': None, 'p10': None, 'p90': None}
			continue
		update_invprop_table(db, pred, value_dist_dict)
		# if (i%100 == 0):
			# print('%d predicates inserted!'%(i+1)) 
